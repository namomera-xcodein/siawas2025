...
include 'ensure_upload_dirs.php';
                ...
                Swal.fire({
                    ...
                    <div class="form-actions alert-danger">
                        <div class="text-right">
                            ...
                            <a href="index.php?page=permohonan_proses" class="btn btn-primary">Kembali</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-right">
                    <div class="card-body">
                        <div class="d-flex d-lg-flex d-md-block align-items-center">
                            <?php
                            $permohonan_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

                            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                                $keterangan_bukti = $_POST['keterangan'] ?? '';
                                $file_nota = $_FILES['bukti_nota_pdf'];
                                $file_foto = $_FILES['foto_geotagging'];
                                $file_dok  = $_FILES['dokumen_pendukung'];
                                ...
                                // Fungsi aman untuk membuat nama file
                                function safeFileName($filename)
                                {
                                    $filename = preg_replace('/[^a-zA-Z0-9\.\-_]/', '_', $filename); // hanya huruf, angka, titik, dash, underscore
                                    return time() . "_" . $filename;
                                }

                                // Proses upload file jika ada
                                if (!empty($file_nota['name'])) {
                                    $safeName = safeFileName($file_nota['name']);
                                    $target = $baseUpload . "nota/" . $safeName;
                                    if (move_uploaded_file($file_nota["tmp_name"], $target)) {
                                        $filePaths['bukti_nota_pdf'] = $safeName;
                                    }
                                }

                                if (!empty($file_foto['name'])) {
                                    $safeName = safeFileName($file_foto['name']);
                                    $target = $baseUpload . "foto/" . $safeName;
                                    if (move_uploaded_file($file_foto["tmp_name"], $target)) {
                                        $filePaths['foto_geotagging'] = $safeName;
                                    }
                                }

                                if (!empty($file_dok['name'])) {
                                    $safeName = safeFileName($file_dok['name']);
                                    $target = $baseUpload . "dok/" . $safeName;
                                    if (move_uploaded_file($file_dok["tmp_name"], $target)) {
                                        $filePaths['dokumen_pendukung'] = $safeName;
                                    }
                                }
                                ...
                                $sql = "UPDATE permohonan SET 
                                        keterangan_bukti = ?, 
                                        bukti_nota_pdf = COALESCE(NULLIF(?, ''), bukti_nota_pdf),
                                        foto_geotagging = COALESCE(NULLIF(?, ''), foto_geotagging),
                                        dokumen_pendukung = COALESCE(NULLIF(?, ''), dokumen_pendukung)
                                        WHERE id = ?";
                                ...
                                $stmt->bind_param(
                                    "ssssi",
                                    $keterangan_bukti,
                                    $filePaths['bukti_nota_pdf'],
                                    $filePaths['foto_geotagging'],
                                    $filePaths['dokumen_pendukung'],
                                    $permohonan_id
                                );
                                $stmt->execute();

                                echo "<script>
                                            Swal.fire('Berhasil', 'Data bukti telah diperbarui!', 'success');
                                        </script>";
                            }
                            ?>
                    ...
                    ?>
                    <div class="card-body">
                        <h4 class="card-title">Kelengkapan Data :</h4>
                        <form action="<?php echo "index.php?page=detail_proses_belanja&id=" . $permohonan_id; ?>"
                            method="POST" enctype="multipart/form-data">
                            <div class="form-group">
                                <label>Nota/Kwitansi Pembelian</label>
                                <?php if (!empty($data['bukti_nota_pdf'])): ?>
                                <a href="<?= $base_url ?>/uploads/nota/<?= urlencode($data['bukti_nota_pdf']) ?>"
                                    target="_blank">
                                    ✅sudah diupload <i class="fa fa-file-pdf"></i> Lihat
                                </a><br>
                                <?php else: ?>
                                <b>Belum ada file yang diupload</b>
                                <?php endif; ?>
                                <input type="file" class="form-control" name="bukti_nota_pdf"
                            ...
                            <div class="form-group">
                                ...
                                <a href="<?= $base_url ?>/uploads/foto/<?= urlencode($data['foto_geotagging']) ?>"
                                    target="_blank">
                                    ✅sudah diupload <i class="fa fa-file-image"></i> Lihat
                                </a><br>
                                <?php else: ?>
                                <b>Belum ada file yang diupload</b>
                                <?php endif; ?>
                                <input type="file" class="form-control" name="foto_geotagging"
                                    accept=".pdf,.jpg,.jpeg,.png">
                                <small class="form-text text-muted">Format: PDF, JPG, PNG (Max 2MB)</small>
                            </div>
                            <div class="form-group">
                                ...
                                <a href="<?= $base_url ?>/uploads/dok/<?= urlencode($data['dokumen_pendukung']) ?>"
                                    target="_blank">
                                    ✅ sudah diupload <i class="fa fa-file-word"></i>Lihat
                                </a><br>
                                <?php else: ?>
                                <b textcolor="red">Belum ada file yang diupload</b>
                            ...
                            <div class="form-group">
                                ...
                                <small id="name1"
                                    class="badge badge-default badge-info form-text text-white float-left">
                            ...
                            <!-- <div class="form-group">
                                <label>Keterangan</label><br>
                                <small id="name1"
                                    class="badge badge-default badge-info form-text text-white float-left">Tulis
                                    Keterangan jika ingin ubah</small>
                                <text class="form-control" name="keterangan" rows="3"
                                    value="<?= htmlspecialchars($data['keterangan_bukti'])  ?>"></text>
                            </div> -->
